...
class _ReceitasFixasScreenState extends State<ReceitasFixasScreen> {
  ...
  final FixedIncomePreferencesService _prefsService = FixedIncomePreferencesService();
  ...
  static Widget _buildCategoryDropdown({
    ...
    final value = category == kIncluirNova || incomeCategories.contains(category)
        ? category
        : (incomeCategories.isNotEmpty ? incomeCategories.first : kIncluirNova);
    return DropdownButtonFormField<String>(
      isExpanded: true,
      value: options.contains(value) ? value : (incomeCategories.isNotEmpty ? incomeCategories.first : kIncluirNova),
  ...
  String _subtitleFixedIncome(Map<String, dynamic> e, int day, DateTime? start, DateTime? end, {bool includeCategory = true}) {
  ...
  Future<void> _openForm({Map<String, dynamic>? existing}) async {
    ...
    final descriptionSuggestionsAll = descSuggestions.toList()..sort(UserCategoriesService.compareNamesPt);
    final descriptionSuggestionsShown =
        descriptionSuggestionsAll.length > 32 ? descriptionSuggestionsAll.sublist(0, 32) : descriptionSuggestionsAll;
    ...
    String category = existing?['category']?.toString() ?? (_incomeCategories.isNotEmpty ? _incomeCategories.first : 'Receita');
    ...
    String mode = (existing?['mode'] ?? FixedIncomeService.modePeriod).toString();
                ...
                appBar: AppBar(
                  ...
                  title: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          isEdit ? 'Editar receita fixa' : 'Nova receita fixa',
                    ...
                    children: [
                        RepaintBoundary(
                          child: FastTextField(
                            ...
                            decoration: _fixedFlowPremiumInputDeco(
                              labelText: 'Descrição',
                              hintText: 'Ex: Aluguel recebido, comissões, juros',
                        ...
                        if (descriptionSuggestionsShown.isNotEmpty) ...[
                          ...
                          Align(
                            ...
                            child: Text(
                              ...
                              style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.grey.shade700, height: 1.25),
                            ),
                          ),
                          ...
                          SizedBox(
                            ...
                            child: ListView.separated(
                              ...
                              itemBuilder: (context, i) {
                                ...
                                return ActionChip(
                                  label: Text(s, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12.5)),
                                  onPressed: () {
                                    descCtrl.text = s;
                                    setModalState(() {});
                                  },
                          ...
                          onCategoryAdded: () async {
                            ...
                            final added = await showDialog<bool>(
                              ...
                              builder: (ctx) => AlertDialog(
                                title: const Text('Nova categoria de receita'),
                            ...
                            nameCtrl.dispose();
                            try {
                              await UserCategoriesService().addCustom(_fsUid, true, name);
                              if (!context.mounted) return;
                              final c = await UserCategoriesService().load(_fsUid);
                              if (!context.mounted) return;
                              final list =
                                  UserCategoriesService.sortedWithoutIncluirNova(c.income);
                              setModalState(() {
                                category = name;
                        ...
                        Container(
                          ...
                          child: SegmentedButton<String>(
                            segments: const [
                              ButtonSegment(value: FixedIncomeService.modePeriod, label: Text('Por período'), icon: Icon(Icons.date_range_rounded, size: 20)),
                              ButtonSegment(value: FixedIncomeService.modeInstallments, label: Text('Por parcelas'), icon: Icon(Icons.receipt_long_rounded, size: 20)),
                            ],
                            selected: {mode},
                            onSelectionChanged: (Set<String> sel) => setModalState(() {
                              ...
                              mode = sel.first;
                              if (mode == FixedIncomeService.modeInstallments) {
                                if (prev != FixedIncomeService.modeInstallments) {
                                  totalParcelas = 12;
                                  parcelaInicial = 1;
                        ...
                        ListTile(
                          ...
                          title: Text(mode == FixedIncomeService.modeInstallments ? 'Data da primeira parcela (que você controla)' : 'Data início'),
                        ...
                        _buildSuperPremiumActionButton(
                          ...
                          onPressed: () async {
                            ...
                            if (mode == FixedIncomeService.modeInstallments && (totalParcelas < 1 || parcelaInicial < 1 || parcelaInicial > totalParcelas)) {
                            ...
                            try {
                              if (isEdit && id != null) {
                                final updatedCount = await _service.update(
                                  ...
                                  mode: mode,
                                  totalParcelas: mode == FixedIncomeService.modeInstallments ? totalParcelas : null,
                                  parcelaInicial: mode == FixedIncomeService.modeInstallments ? parcelaInicial : null,
                                );
                                if (context.mounted) {
                                  if (updatedCount > 0) {
                                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Receita fixa atualizada. $updatedCount parcela(s) futura(s) ajustada(s) para o novo dia.')));
                                  } else {
                                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Receita fixa atualizada.')));
                                  }
                                }
                              } else {
                                await _service.add(
                                ...
                                if (context.mounted) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(content: Text('Receita fixa criada. Gerando lançamentos no Financeiro…')),
                                  );
                                }
                              }
          ...
          child: Column(
            ...
            children: [
              ...
              SwitchListTile(
                ...
                title: const Text('Mostrar receitas fixas nas receitas pendentes', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
              ...
              Padding(
                ...
                child: Wrap(
                  ...
                  children: [
                    ...
                    const Text(' nas receitas pendentes', style: TextStyle(fontSize: 14, color: AppColors.textSecondary)),
                  ],
                ),
              ),
            ],
  ...
  Widget _buildSuperPremiumActionButton({
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: const Color(0xFF14532D).withValues(alpha: 0.4), blurRadius: 16, offset: const Offset(0, 6)),
          BoxShadow(color: AppColors.financeReceita.withValues(alpha: 0.28), blurRadius: 10, offset: const Offset(0, 3)),
        ],
      ),
      child: Material(
  ...
  Future<void> _delete(Map<String, dynamic> item) async {
    ...
    try {
      ...
      if (removeParcelas) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Receita fixa excluída e $count lançamento(s) removido(s) do Financeiro.')),
          );
        }
      } else {
        await _service.delete(_fsUid, id);
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Receita fixa excluída. Os lançamentos já criados permanecem.')));
      }
        ...
        builder: (context, snap) {
          ...
          return CustomScrollView(
            ...
            slivers: [
              ...
              final vis = financeCategoryVisualFor(catName, isIncome: true);
              ...
              return Padding(
                ...
                child: Material(
                  ...
                  leading: financeCategoryLeadingTile(catName, isIncome: true),
                  title: Text(
                    catName,
                  ...
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Text(CurrencyFormats.formatBRL(amount), style: const TextStyle(fontWeight: FontWeight.w900, color: Color(0xFF15803D), fontSize: 15)),
